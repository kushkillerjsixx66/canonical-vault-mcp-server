...
Read-only MCP server that exposes the `canonical-vault` GitHub repo to any MCP client — GitHub Copilot, Claude, Grok, ChatGPT, or anything else that speaks Streamable HTTP MCP.
...
Scope for both is fixed by the Model Contribution Contract (`00_governance/contracts/model-contribution.md`) and `canonical_merge_authority: false`: no direct writes to the canonical branch, no path in a prohibited zone, no merge tool anywhere in this server. See "Write tools" below for setup.
...
| `GITHUB_WRITE_BRANCH_ALLOWLIST` | No | `grok,claude,chatgpt,gemini,copilot` | Comma-separated branches the write tools may touch. Matches all existing model branches in canonical-vault. The server refuses to boot if this ever includes `GITHUB_DEFAULT_REF`. |
...
Everything here is governed by the Model Contribution Contract and `canonical_merge_authority: false`:

- **Branch sovereignty** — Each model is expected to write only to its own branch (`grok` → `grok`, `claude` → `claude`, `chatgpt` → `chatgpt`, etc.). The shared allowlist is the governed surface; clients must pass their own branch name.
- **No merge authority** — There is no merge tool, ever. `vault_open_pr` opens a PR; a human merges it on GitHub.
- **Prohibited zones** → `WRITE_PATH_DENYLIST` in `src/constants.ts` blocks the constitution, IP/legal docs, and the Claude manifest itself, regardless of branch.
- **Hard startup guard** — The server throws on startup if `main` (or whatever `GITHUB_DEFAULT_REF` is) is ever added to the allowlist — a misconfigured env var fails loud, not silent.
...
**Client expectation (option A):**
When calling `vault_propose_change` or `vault_open_pr`, each model must set `branch` / `head` to its own identity branch. Cross-branch writes are refused by the allowlist only if the branch is not listed; the contract (and operator review) is what keeps Grok off the Claude branch and vice versa.
...
- **Write tools are the separate, reviewed addition** — `src/services/github-write.ts` and `src/tools/vault-write.ts` are standalone modules; nothing in the existing read-only path was touched. They only activate when `GITHUB_WRITE_TOKEN` is explicitly set. Enforcement is layered: PAT scoped to Contents on this repo alone, GitHub branch protection on `main`, and the tool code refuses out-of-scope branches or paths before it ever calls GitHub. There is no merge tool anywhere in this server — `vault_open_pr` only opens the PR.
