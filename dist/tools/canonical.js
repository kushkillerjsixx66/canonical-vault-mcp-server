...
import { CANONICAL_RESOURCES, fetchCanonicalContent, resourceUri, } from "../resources/canonical.js";
export function registerCanonicalTools(server) {
    const names = CANONICAL_RESOURCES.map((r) => r.name);
    server.registerTool("vault_get_canonical", {
...
Prefer this over vault_get_file for constitution, invariants, gates, module-registry, node-model, neuralese, index, etc.`,
        ...
        try {
            const { text, sha, size, truncated } = await fetchCanonicalContent(resource, params.ref);
            return {
