...
import { CANONICAL_RESOURCES, fetchCanonicalContent, findCanonicalByUri, handleGitHubError, resourceUri, } from "./resources/canonical.js";
...
export function createServer() {
    ...
    for (const r of CANONICAL_RESOURCES) {
        ...
        const register = typeof server.registerResource === "function"
            ? server.registerResource.bind(server)
        ...
        register(r.name, uri, {
            ...
            try {
                const resource = findCanonicalByUri(uriStr) ??
                        ...
                        contents: [
                            {
                                uri: uriStr,
                                mimeType: "text/plain",
                                text: `Error: unknown resource URI '${uriStr}'. Known: ${CANONICAL_RESOURCES.map((c) => resourceUri(c.name)).join(", ")}`,
                            },
                        ],
                    };
                }
                const { text } = await fetchCanonicalContent(resource, GITHUB_DEFAULT_REF);
                return {
                    contents: [
                        {
    ...
    if (typeof server.registerPrompt === "function") {
        server.registerPrompt("vault_orientation", {
            title: "Vault Orientation",
            description: "Orientation for working with the canonical Lattice vault: which resources to read first and altitude rules.",
        }, async () => ({
            messages: [
                {
                    role: "user",
                    content: { type: "text", text: orientationText },
                },
            ],
        }));
    }
        ...
        server.prompt("vault_orientation", "Orientation for working with the canonical Lattice vault: which resources to read first and altitude rules.", async () => ({
            messages: [
                {
                    role: "user",
                    content: { type: "text", text: orientationText },
